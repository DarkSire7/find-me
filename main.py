...
Endpoints:
  ...
  POST /match/faces    — Face encoding comparison via insightface ArcFace
  POST /extract-tags   — Physical tag extraction via Gemini 1.5 Flash
"""
...
def location_score(found: Optional[GeoPoint], missing: Optional[GeoPoint]) -> tuple[float, Optional[float]]:
    ...
    return math.exp(-dist / SIGMA), dist
...
def age_score(found_age: Optional[int], missing_age: Optional[int]) -> float:
    ...
    return math.exp(-(diff ** 2) / (2 * 7.0 ** 2))
...
def compute_composite_score(found: PersonRecord, missing: PersonRecord) -> MatchResult:
...
def _download_image_bytes(url: str, timeout: int = 10) -> bytes:
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "DisasterIQ-ML/2.0"})
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(
        ...
        content={
            ...
            "message": str(exc) if os.getenv("DEBUG") == "true" else "Check server logs.",
        },
    )
...
def match_strings(payload: StringMatchRequest) -> StringMatchResponse:
    ...
    try:
        ...
        logger.exception("Error in match_strings for found=%s: %s", payload.found_person.id, exc)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"error": "SCORING_FAILED", "message": str(exc)},
        )

    return StringMatchResponse(found_person_id=payload.found_person.id, matches=results)
...
def match_faces(payload: FaceMatchRequest) -> FaceMatchResponse:
    if face_app is None:
        raise HTTPException(
            ...
            detail="Face recognition backend is not available in this deployment.",
        )
    ...
    return FaceMatchResponse(
        ...
        face_distance=round(1.0 - cos_sim, 4),
...
@app.post(
    ...
    tags=["AI"],
    summary="Extract physical descriptors from a person's image using Gemini 1.5 Flash.",
)
async def extract_tags(payload: TagExtractionRequest) -> TagExtractionResponse:
    if gemini_model is None:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Gemini AI is not configured (missing GEMINI_API_KEY).",
        )
    ...
    try:
        ...
        if not response or not hasattr(response, "text") or not response.text:
            return TagExtractionResponse(
                physical_tags=[],
                warning="AI returned an empty response (may be safety filter).",
            )
        ...
        logger.error("Gemini tag extraction failed: %s", exc)
        return TagExtractionResponse(physical_tags=[], error=str(exc))


# ---------------------------------------------------------------------------
# Entry-point (local dev only — Cloud Run uses uvicorn via CMD in Dockerfile)
# ---------------------------------------------------------------------------
